...
export class ApplicationJobMapper {
  ...
  static toEntity(command: any): JobEntity {
    return new JobEntity({ _id: command.id, name: command.name, status: command.status, export_format: command.export_format, collection: command.collection, query: command.query.map(ApplicationQueryMapper.toEntity), fields: command.fields, sort: command.sort, file_url: command.file_url, created_at: command.created_at, updated_at: command.updated_at });
  }
}
